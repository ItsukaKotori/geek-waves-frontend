...
function run() {
  ...
  output.value = ''
  try {
    output.value = radixConvert(input.value, from.value, to.value)
  } catch (e) {
...
<template>
  <div class="flex flex-col gap-3">
    ...
    <input v-model="input" placeholder="如 ff(十六进)、1010(二进)或 42(十进制)" class="input input-sm font-mono" />
    <div class="flex items-center gap-2">
      <select v-model="from" class="select select-sm w-28">
        <option v-for="b in bases" :key="b" :value="b">{{ b }} 进制</option>
      </select>
      <span class="text-base-content/60">→</span>
      <select v-model="to" class="select select-sm w-28">
        <option v-for="b in bases" :key="b" :value="b">{{ b }} 进制</option>
      </select>
